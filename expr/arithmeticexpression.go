...
func arithmeticExpression() recursiveExpression {
	...
	power := and(ws, &operand, ws, zeroOrMore(and(ws, exp, ws, &operand, ws)))
	term := and(ws, power, ws, zeroOrMore(and(ws, or(mul, div), ws, power, ws)))
	expression.value = and(ws, term, ws, zeroOrMore(and(ws, or(add, sub), ws, term, ws)))
